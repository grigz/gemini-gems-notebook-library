...
function updateGem(rowId, gemData) {
  try {
    ...
    if (!canUserEditEntry(rowId)) {
      Logger.log('Unauthorized edit attempt by: ' + userEmail + ' for row: ' + rowId);
      return {
        success: false,
        data: null,
        error: 'You do not have permission to edit this entry. Only the creator or administrators can edit entries.'
      };
    }
...
/**
 * Delete a gem (creator or admin)
 * @param {number} rowId - The row number (1-based)
 * @return {Object} Response object with success status
 */
function deleteGem(rowId) {
  // Check if user has permission to delete this entry (creator or admin)
  if (!canUserEditEntry(rowId)) {
    Logger.log('Unauthorized delete attempt by: ' + getUserEmail() + ' for row: ' + rowId);
    return {
      success: false,
      data: null,
      error: 'You do not have permission to delete this entry. Only the creator or administrators can delete entries.'
    };
  }
 ...
 * @param {number} rowId - The row number (1-based)
 * @return {boolean} True if user can edit (is creator or admin)
 */
function canUserEditEntry(rowId) {
  ...
  try {
    ...
    // Creator can edit their own entry
    return userEmail.toLowerCase() === createdBy.toLowerCase();
  } catch (error) {
    Logger.log('Error checking edit permissions: ' + error.toString());
    return false;
  }
}
